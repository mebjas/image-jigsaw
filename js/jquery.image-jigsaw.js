...
(function( $ ) {
	$.fn.jigsaw = function(options)
	{
		...
		for(i=0;i<$.fn.jigsaw.defaults.x;i++) {
			for(j=0;j<$.fn.jigsaw.defaults.y;j++) {
				...
				obj.children("div[pos='" +pos +"']")
							.css("background-position", "-" +(j*w) +"px -" +(i*h) +"px")
							.css("width", w +"px")
							.css("height", h+"px")
							.css("background-image", "url("+$.fn.jigsaw.defaults.image+")")
							.css("display", "inline-block")
							.css("margin", $.fn.jigsaw.defaults.margin)
	...
	function animate(obj)
	{
		...
		var selected = [];
		if ($.fn.jigsaw.defaults.distinct) {
			for (i = 0; i < $.fn.jigsaw.defaults.x; i++) {
				selected[i] = [];
				for (j = 0; j < $.fn.jigsaw.defaults.y; j++) {
					selected[i][j] = false;
		...
		for(i = 0; i < len; i++) {
			var randI = Math.floor((Math.random() * 1000)) % $.fn.jigsaw.defaults.x;
			var randJ = Math.floor((Math.random() * 1000)) % $.fn.jigsaw.defaults.y;

			if ($.fn.jigsaw.defaults.distinct && selected[randI][randJ]) {
				for (x = 0; x < $.fn.jigsaw.defaults.x; x++) {
					for (y = 0; y < $.fn.jigsaw.defaults.y; y++) {
						if (!selected[x][y]) {
							randI = x;
							randJ = y;
							break;
		...
		} else t = setTimeout(function(){animate(obj);},$.fn.jigsaw.defaults.freq);
	}
